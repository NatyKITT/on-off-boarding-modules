...
import { sendProbationEvaluationReminderEmail } from "@/lib/email"
import {
...
export async function POST(
  ...
  try {
    ...
    if (completed && !revisionOpen) {
      return jsonError(
        "Hodnocení už je vyplněné. Připomínku vedoucímu lze poslat jen po otevření formuláře k úpravě.",
        409
      )
    }
    ...
    await prisma.$transaction(async (tx) => {
      await tx.probationEvaluationRequest.update({
        ...
        data: {
          ...
          supervisorEmail,
          lastReminderAt: now,
          lastReminderBy: userKey,
          lastReminderByName: userLabel,
          reminderCount: {
            increment: 1,
          },
        },
      ...
      await addProbationEvent(tx, {
        ...
        message: revisionOpen
          ? `Připomínka k úpravě vyhodnocení zkušební doby byla ručně odeslána na ${supervisorEmail}.`
          : `Připomínka k hodnocení zkušební doby byla ručně odeslána na ${supervisorEmail}.`,
        meta: {
          source: "manual",
          directSend: true,
    ...
    if (!updated) {
      return jsonError(
        "Připomínka byla odeslána, ale nepodařilo se znovu načíst stav formuláře.",
        500
      )
    }
    ...
    console.error("[PROBATION REMIND]", error)

    return NextResponse.json(
      {
        status: "error",
        message: "Připomínku se nepodařilo odeslat.",
