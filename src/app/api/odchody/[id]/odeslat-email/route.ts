...
export async function POST(
  ...
  const rec = await prisma.employeeOffboarding.findUnique({ where: { id } })
  if (!rec)
    return NextResponse.json(
  ...
  const recipients = Array.from(
    new Set(
      [overrideEmail, rec.userEmail ?? undefined].filter(Boolean) as string[]
    )
  )
  ...
  await prisma.$transaction([
    prisma.mailQueue.create({
      data: {
        type: "EMPLOYEE_INFO",
        payload: { kind: "offboarding", id: rec.id, to: recipients },
        status: "QUEUED",
        createdBy,
      },
    }),
    prisma.offboardingChangeLog.create({
      data: {
        employeeId: rec.id,
        userId: createdBy,
